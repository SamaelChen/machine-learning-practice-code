...
#     conv3 = mx.sym.Convolution(data=relu2, kernel=(3, 3), num_filter=128)
...
class WriteScalar(object):
    ...
    def __call__(self, param):
        ...
        for name, value in name_value:
            logging.info('Epoch[%d] Validation-%s=%f Step=%d', param.epoch, name, value, param.nbatch)
            s = scalar(name, value)
            writer.add_summary(s, param.nbatch)
...
model.fit(train_data=data_train, eval_data=data_test, optimizer='sgd',
          ...
          batch_end_callback=[mx.callback.Speedometer(batch_size, 50), WriteScalar()])
